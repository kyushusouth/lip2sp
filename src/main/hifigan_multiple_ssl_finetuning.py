...
from src.pl_datamodule.with_speech_ssl import WithSpeechSSLDataModule
from src.pl_module.hifigan_multiple_ssl import LitHiFiGANMultipltSSLModelFineTuning
...
def main(cfg: omegaconf.DictConfig) -> None:
    ...
    datamodule = WithSpeechSSLDataModule(cfg)

    if cfg.training.finetune:
        model = LitHiFiGANMultipltSSLModelFineTuning.load_from_checkpoint(
            checkpoint_path=cfg.training.finetune_start_model_path,
            strict=False,
            cfg=cfg,
        )
    else:
        model = LitHiFiGANMultipltSSLModelFineTuning(cfg)
