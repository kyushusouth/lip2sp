...
from src.pl_datamodule.layerwise_asr import LayerwiseASRDataModule
from src.pl_module.layerwise_asr import LayerwiseASRModule
...
def main(cfg: omegaconf.DictConfig) -> None:
    ...
    datamodule = LayerwiseASRDataModule(cfg)

    if cfg.training.finetune:
        model = LayerwiseASRModule.load_from_checkpoint(
            checkpoint_path=cfg.training.finetune_start_model_path, cfg=cfg
        )
    else:
        model = LayerwiseASRModule(cfg=cfg)
