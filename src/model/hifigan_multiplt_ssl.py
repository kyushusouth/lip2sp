...
class Generator(torch.nn.Module):
    def __init__(self, cfg):
        ...
        if "hubert_final_feature_cluster" in cfg.model.hifigan.input:
            model_in_dim += cfg.model.hifigan.embedding_dim
        if "wav2vec2_final_feature_cluster" in cfg.model.hifigan.input:
            model_in_dim += cfg.model.hifigan.embedding_dim
        if "data2vec_final_feature_cluster" in cfg.model.hifigan.input:
            model_in_dim += cfg.model.hifigan.embedding_dim
        ...
        self.emb_hubert_final_feature_cluster = nn.Embedding(
            cfg.model.decoder.speech_ssl.n_clusters + 1,
            cfg.model.hifigan.embedding_dim,
        )
        self.emb_wav2vec2_final_feature_cluster = nn.Embedding(
            cfg.model.decoder.speech_ssl.n_clusters + 1,
            cfg.model.hifigan.embedding_dim,
        )
        self.emb_data2vec_final_feature_cluster = nn.Embedding(
            cfg.model.decoder.speech_ssl.n_clusters + 1,
            cfg.model.hifigan.embedding_dim,
        )
    ...
    def forward(
        ...
        if "hubert_final_feature_cluster" in self.cfg.model.hifigan.input:
            features.append(
                self.emb_hubert_final_feature_cluster(
                    inputs_dict["hubert_final_feature_cluster"]
                )
            )
        if "wav2vec2_final_feature_cluster" in self.cfg.model.hifigan.input:
            features.append(
                self.emb_wav2vec2_final_feature_cluster(
                    inputs_dict["wav2vec2_final_feature_cluster"]
                )
            )
        if "data2vec_final_feature_cluster" in self.cfg.model.hifigan.input:
            features.append(
                self.emb_data2vec_final_feature_cluster(
                    inputs_dict["data2vec_final_feature_cluster"]
                )
            )
