...
from src.collate_fn.base_hubert_2 import adjust_seq_lengths
from src.dataset.base_hubert_2 import BaseHuBERT2Dataset
...
class BaseHuBERT2DataModule(L.LightningDataModule):
    ...
    def setup(self, stage: str) -> None:
        ...
        if stage == "fit":
            self.train_dataset = BaseHuBERT2Dataset(
                cfg=self.cfg,
                data_path_list=train_data_path_list,
                transform=BaseHuBERT2Transform(self.cfg, "train"),
            )
            self.val_dataset = BaseHuBERT2Dataset(
                cfg=self.cfg,
