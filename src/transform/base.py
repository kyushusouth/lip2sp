...
class BaseTransform:
    ...
    def random_crop(self, lip, center):
        ...
        """
        if center:
            top = left = (self.cfg.model.imsize - self.cfg['data']['video']['imsize_cropped']) // 2
        else:
            top = torch.randint(
                0, self.cfg.model.imsize - self.cfg['data']['video']['imsize_cropped'], (1,)
            )
            left = torch.randint(
                0, self.cfg.model.imsize - self.cfg['data']['video']['imsize_cropped'], (1,)
            )
    ...
    def segment_masking_segmean(self, lip):
        ...
        mask_length = torch.randint(
            0, int(self.cfg.model.fps * self.cfg.train.max_segment_masking_sec), (1,)
        )
    ...
    def __call__(
        ...
        feature_avhubert = self.stacker(
            feature_avhubert, self.cfg.model.reduction_factor
        )  # (T // reduction_factor, C * reduction_factor)
